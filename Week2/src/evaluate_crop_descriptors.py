...
from params import best_config_descriptors as experiments
...
def main(data_dir: Path, generate_plots=False) -> None:
    ...
    methods = experiments["color_space"]
    metrics = experiments["metric"]
    bins = experiments["n_bins"]
    k_values = experiments["k_values"]
    n_crops = experiments["n_crops"]
    ...
    for k in k_values:
        ...
        for n_crop in n_crops:
            for method in methods:
                for n_bins in bins:
                    bbdd_descriptors = read_pickle(SCRIPT_DIR / "descriptors" / f"{method}_{n_bins}bins_{n_crop}crops_noPyramid_descriptors.pkl")
                    ...
                    for metric in metrics:
                        ...
                        if generate_plots:
                            ...
                            plot_descriptors_difference(img_descriptors, best_descriptors,
                                        save_path=SCRIPT_DIR / 'outputs' / f'descriptor_difference_at{k}_{method}_{n_bins}_{metric}_{n_crop}.png')
                            
                            # Plot the results with similarity values
                            plot_query_results(images, results_indices, results_similarities, k=k,
                                        save_path=SCRIPT_DIR / 'outputs' / f'query_at{k}_{method}_{n_bins}_{metric}_{n_crop}.png')
        ...
        if matrix:
            for method in methods:
                for metric in metrics:
                    ...
                    plt.tight_layout()
                    plt.savefig(
                        SCRIPT_DIR / "outputs" / f"map{k}_matrix_{method}_{metric}_TrueWeights.png"
                    )
                    plt.close()
        else:
            for method in methods:
                for metric in metrics:
                    ...
                    plt.tight_layout()
                    plt.savefig(
                        SCRIPT_DIR / "outputs" / f"map{k}_function_crops_xaxis_{method}_{metric}__TrueWeights.png"
                    )
                    plt.close()
