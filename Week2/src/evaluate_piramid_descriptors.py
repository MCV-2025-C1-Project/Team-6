...
def main(data_dir: Path, generate_plots=False) -> None:
    ...
    piramid_levels = experiments["pyramid_levels"]
            ...
            for method in methods:
                for n_bins in bins:
                    ...
                    for metric in metrics:
                        ...
                        mapk_scores[f"{method}_{n_bins}bins_{metric}_{piramid_level}"] = map_score
        ...
        if matrix:
            for method in methods:
                for metric in metrics:
                    ...
                    ax.set_yticks(np.arange(len(piramid_levels)))
                    ax.set_xticklabels(bins)
                    ax.set_yticklabels(piramid_levels)
                    ax.set_xlabel("Number of bins")
                    ax.set_ylabel("Number of crops")
                    ax.set_title(f"MAP@{k} for {method} - {metric}")

                    # Annotate cells with values
                    for i in range(len(piramid_levels)):
            ...
            for method in methods:
                for metric in metrics:
                    ...
                    for j, n_bin in enumerate(bins):
                        ...
                        x_data = np.array(piramid_levels) # The x-axis data
